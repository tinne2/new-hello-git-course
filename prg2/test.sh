...
run_perf_tests() {
    ...
    if [ -f "${EXECUTABLE_PROGRAM}" ]; then
        ...
        echo "perftest 20*get_next_bites_from 5 $first:$last:$step" >>$file
        echo "perftest 20*get_connections 5 $first:$last:$step" >>$file
        echo "perftest 20*path_any 5 $first:$last:$step" >>$file
...
run_opt_perf_tests() {
    ...
    if [ -f "${EXECUTABLE_PROGRAM}" ]; then
        ...
        local step=1000
        echo "random_seed ${RANDOM_SEED}" >$file
        echo "perftest 20*path_shortest 5 $first:$last:$step" >>$file
        echo "perftest 20*path_least_bites 5 $first:$last:$step" >>$file
        echo "perftest 20*path_least_uphill 5 $first:$last:$step" >>$file
        timeout 120 ./${EXECUTABLE_PROGRAM} $file
        rm $file
    else
        echo "prg2 executable could not be found or built."
